...
xxd -r -p <(sed -n '1,8704'p firmware.xxd      | sed "s/ //g" | tr -d '\n') > j20s_novatek_ts_fw01.bin
xxd -r -p <(sed -n '8705,17408'p firmware.xxd  | sed "s/ //g" | tr -d '\n') > j20s_novatek_ts_mp01.bin
xxd -r -p <(sed -n '17409,26112'p firmware.xxd | sed "s/ //g" | tr -d '\n') > j20s_novatek_ts_fw02.bin
